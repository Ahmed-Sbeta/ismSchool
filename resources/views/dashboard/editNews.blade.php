...
<body class="g-sidenav-show bg-gray-200">
  ...
  <div class="main-content position-relative max-height-vh-100 h-100">
    ...
    <div class="container-fluid px-2 px-md-4">
      ...
      @endif
    <div class="container">
      <form class="form-control" action="{{url('/updateNews/'.$News->id)}}" method="post" enctype="multipart/form-data">
        @csrf
        @method('PATCH')
        <label for="" class="col-1">Title</label>
        <input type="text" name="title" class="col-6" value="{{$News->title}}">
        <br>
        <label for="" class="col-1">Date</label>
        <input type="date" name="date"  class="col-6" value="{{$News->date}}">
        <br>
        <label for="" class="col-1 float-start">text</label>
        <textarea name="text" class="col-9" rows="8" cols="80">{{$News->text}}</textarea>
        <br>
        <label for="" class="col-1">image</label>
        <input type="file" class="col-6" name="image" value="{{$News->image}}">
